...
**The active Claude session is the coding agent.** Unlike the legacy headless mode (where `orchestrate.py` spawned `claude --print` subprocesses), here *you* — the session with full codebase + conversation context — do the implementing and fixing. The Python script at `/home/john.noble/dev/code-orchestrator/orchestrate.py` is used only as a stateless review tool: `--review-model MODEL` returns one outside model's findings on the current branch diff.
...
```bash
cd /home/john.noble/dev/code-orchestrator && \
python orchestrate.py {TICKET} {WORKTREE} --review-model {MODEL} --base-branch develop
```
...
- **Legacy headless mode** still exists: `orchestrate.py {TICKET} {WORKTREE} --start-from 2` runs the full pipeline with `claude --print` subprocesses. Use that only for unattended/background runs.
