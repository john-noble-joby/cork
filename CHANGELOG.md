...
- `install.sh` now replaces each installed skill directory instead of merging into it, stages each skill in a temp dir, keeps the old copy until the new one is in place, and refuses a destination that overlaps the repo's own `skills/` tree before creating it.
